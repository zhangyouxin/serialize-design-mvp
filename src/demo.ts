...
const withdrawalReqeustPackable = createRawWithdrawalRequestPackable({ 
  nonce: {
    normalize: (value: BI) => NumberConvertor.BIToUint32Buffer(value), // dummy
    denormalize:(buf: ArrayBuffer) => NumberConvertor.Uint32BufferToBI(buf), // dummy
  },
  capacity: {
    normalize: (value: BI) => NumberConvertor.BIToUint64Buffer(value), // dummy
    denormalize:(buf: ArrayBuffer) => NumberConvertor.Uint64BufferToBI(buf), // dummy
  },
  fee: {
    sudt_id: {
      normalize: (value: BI) => NumberConvertor.BIToUint32Buffer(value), // dummy
    ...
    amount: {
      normalize: (value: BI) => NumberConvertor.BIToUint128Buffer(value), // dummy
      denormalize:(buf: ArrayBuffer) => NumberConvertor.Uint128BufferToBI(buf), // dummy
    },
  }
})
// 第 4 步，序列化
